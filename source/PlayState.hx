...
class PlayState extends MusicBeatState
{
	...
	var santa:FlxSprite;
	var bgwhat:FlxSprite;
	var funnyrock:FlxSprite;
	var funnyyrock:FlxSprite;
		...
		switch(stageCheck)
		{
			...
			case "wizardstage":
			{
					...
					curStage = 'wizardstage';
						...
						funnyrock = new FlxSprite(0,0).loadGraphic(Paths.image('evilwiz/background/Crystal'));
						...
						funnyyrock = new FlxSprite(0,0).loadGraphic(Paths.image('evilwiz/background/Crystal'));
	...
	var crowdcanappear:Bool = true;
		function crowdmechanic() //had to fix kades code :joy:
			{
				if (crowdcanappear != false)
					{
						crowdcanappear = true;
						var dacrowd:FlxSprite = new FlxSprite(-1000,-515);
						dacrowd.frames = Paths.getSparrowAtlas("Crowdsheet", "barb");
						dacrowd.animation.addByPrefix('cheer','Crowd Mechanic',24,false);
						dacrowd.setGraphicSize(Std.int(crowd.width * 0.48));
						add(dacrowd);
						dacrowd.cameras = [camHUD];
						crowd.animation.play('cheer');
						crowd.animation.finishCallback = function(lol:String)
							{
								remove(crowd);
	...
	*/
	...
	if (curSong.toLowerCase() == 'subjugation') // shitty hard coding i'm too lazy to change 
	{ 
		if (curStep == 255)
		{
	...
	override function beatHit()
	{
		...
		{
		//	if (curSong.toLowerCase() == 'memory-replevy' && curBeat % 4 == 0)
		//	{
		...
		switch (curStage)
		{
			...
			case "icebg":
				{
				if(FlxG.save.data.distractions){
						torch.animation.play('flame', false);
					}
				}
			case 'limo':
				if(FlxG.save.data.distractions){
